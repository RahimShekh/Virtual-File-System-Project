...
UFDT UFDTArr[50];  // MAXINODEcpn lihu shakto
...
void man(char *name)
{
    ...
    {
        printf("Description : Used to close openend file\n");
    ...
    while(1)
    {
     ...
     else if(count == 3)
     {
        ...
        else if(strcmp(command[0],"open") == 0)
        {
            ...
            if(ret >= 0)
                   printf("File is succesfully opend with the file descriptor : %d\n",ret);
            if(ret == -1)
                    printf("ERROR : Incorrect parameters\n");
             if(ret == -2)
